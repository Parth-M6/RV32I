...
def compile_rtl(root: Path, vvp_name: str = "run") -> Path:
    """Compile all rtl/ + tb/ sources into a single vvp image."""
    sources = c.find_verilog_sources(root)
...
def full_pipeline(root: Path, source: Path, *, fmt: str = "words", dis: bool = True,
    ...
    vvp_path = shared_vvp if shared_vvp is not None else compile_rtl(root, result.name)
